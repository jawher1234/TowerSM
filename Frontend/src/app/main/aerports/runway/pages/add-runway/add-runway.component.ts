...
@Component({
  ...
  styleUrls: ['./add-runway.component.css']
})
export class AddRunwayComponent implements OnInit {
  map: google.maps.Map;
...
createRunway(): void {
  ...
  // Set the runway.positions property
  this.runway.positions = allValues;

  // Make the API call
  this.aerportService.createRunway(this.selectedAerportId, this.runway)
    .subscribe({
      next: (res) => {
        console.log(res);
